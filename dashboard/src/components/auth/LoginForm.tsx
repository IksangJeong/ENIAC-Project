...
export function LoginForm() {
  ...
  return (
    <div className="space-y-5 font-mono w-full max-w-sm mx-auto">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            <div className="flex justify-between items-center px-0.5">
              ...
              <div className="flex gap-2 font-mono text-[8px] items-center">
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="text-[var(--color-text-secondary)] hover:text-[var(--color-primary)] transition-colors underline underline-offset-2 opacity-60 hover:opacity-100 uppercase"
                >
                  {showPassword ? "[CONCEAL]" : "[REVEAL]"}
                </button>
                <span className="opacity-20">|</span>
                <Link href="/auth/forgot-password" className="text-[var(--color-text-secondary)] hover:text-[var(--color-primary)] transition-colors underline underline-offset-2 opacity-60 hover:opacity-100">LOST_KEY?</Link>
              </div>
            </div>
            <input
      ...
      {/* Quick Mock Credentials Drawer */}
      <div className="border border-[var(--color-primary)]/10 bg-black/40 p-3 rounded mt-4">
        <button
          type="button"
          onClick={() => setShowMockPanel(!showMockPanel)}
          className="flex justify-between items-center w-full text-[8px] font-mono uppercase tracking-[0.2em] text-[var(--color-primary)]/60 hover:text-[var(--color-primary)] transition-colors"
        >
          <span>[+] DEV_QUICK_LOGIN_DRAWER</span>
          <span>{showMockPanel ? "CLOSE" : "EXPAND"}</span>
        </button>
        ...
        <AnimatePresence>
          {showMockPanel && (
            <motion.div
              ...
              className="overflow-hidden mt-3 space-y-2 border-t border-[var(--color-primary)]/5 pt-3"
            >
              <p className="text-[7px] text-[var(--color-text-secondary)] uppercase">// Pre-fill node identity parameters:</p>
              <div className="grid grid-cols-3 gap-1.5">
                {[
                  { label: "ROOT", user: "dev_admin", pass: "dev123", color: "border-amber-500/30 text-amber-500 hover:bg-amber-500/10" },
                  { label: "OFFICER", user: "officer_admin", pass: "officer123", color: "border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/10" },
                  { label: "MEMBER", user: "user", pass: "user123", color: "border-[var(--color-primary)]/30 text-[var(--color-primary)] hover:bg-[var(--color-primary)]/10" },
                ].map((cred) => (
                  <button
                    key={cred.label}
                    type="button"
                    onClick={() => {
                      setFormData({ username: cred.user, password: cred.pass });
                      setError("");
                    }}
                    className={clsx(
                      "py-1.5 border rounded text-[8px] font-bold font-mono tracking-wider transition-all active:scale-95 text-center uppercase",
                      cred.color
                    )}
                  >
                    {cred.label}
